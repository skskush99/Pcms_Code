...
namespace Authentication.Service.Middleware
{
    public class DecryptionMiddleware
    {
        ...
        public DecryptionMiddleware(RequestDelegate next, IConfiguration configuration)
        {
            _next = next;
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            try
            {
                ...
                if (context.Request.ContentType != null && context.Request.ContentType.StartsWith("application/x-www-form-urlencoded", StringComparison.OrdinalIgnoreCase))
                {
                    await _next(context);
                    return;
                }
                ...
                using (var reader = new StreamReader(context.Request.Body, Encoding.UTF8, true, 1024, true))
                {
                    ...
                    if (!string.IsNullOrEmpty(encryptedData))
                    {
                        ...
                        if (jsonData != null)
                        {
                            string secretDataKey = Convert.ToString(jsonData["data"].Value<string>());
                ...
                using (var newBodyStream = new MemoryStream())
                {
                    ...
                    {
                        // Redirect, skip encryption
                        await newBodyStream.CopyToAsync(originalBodyStream);
                        return;
                    }

                    if (context.Response.ContentType != null && context.Response.ContentType.Contains("text/html", System.StringComparison.OrdinalIgnoreCase))
                    {
                        await newBodyStream.CopyToAsync(originalBodyStream);
                        return;
                    }
                    ...
                    try
                    {
                        ...
                        {
                            // If there's no response, just copy the original body
                            await originalBodyStream.CopyToAsync(originalBodyStream);
                        }
                    }
                    catch (Exception)
                    {
            ...
            catch (Exception ex)
            {
                ...
                await context.Response.WriteAsync(context.Request.ContentType +"   "+ ex.Message + "/n/t" + ex.StackTrace);
                return;
            }
            //}
        }
